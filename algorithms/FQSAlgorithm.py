...
def run(instance, isRealDemand, info, f):
    ...
    if instance['orders'] == {}:
        return
    orderIndex, driverIndex = BaseAlgorithm.buildIndex1(instance)
    ...
    # print(len(instance['orders']), len(instance['drivers']), len(instance['secondDrivers']))
    # print(instance['drivers1'][250]['temp_route'])
    ...
    for k in range(len(instance['drivers'])):
        ...
        selectedOrder = BaseAlgorithm.new_findBestOrders(instance['drivers'][k], zoneOrders, zoneIdleTimeMap, oracle,
        ...
        if instance['drivers'][k] != {} and selectedOrder != {}:
            ...
            zoneDriverCount[instance['drivers'][k]['temp_route'][0][0]] = zoneDriverCount[instance['drivers'][k]['temp_route'][0][0]] - 1
            ...
            # TaxiDemandSupplyOracle.addTimeRecord(instance['taxiWatchdog'], instance['node2region'][
            #     instance['drivers'][k]['temp_route'][len(instance['drivers'][k]['temp_route']) - 1][0]],
            #                                      currentTimeOffset + instance['drivers'][k]['temp_route'][
            #                                          len(instance['drivers'][k]['temp_route']) - 1][2], 1)

            # TaxiDemandSupplyOracle.addTimeRecord(instance['taxiWatchdog'], instance['drivers'][k]['temp_route'][len(instance['drivers'][k]['temp_route'])-1][0]+1,
            #                                      instance['drivers'][k]['temp_route'][len(instance['drivers'][k]['temp_route'])-1][2], 1)
            # zoneIdleTimeMap.pop(selectedOrder['endZoneID'])
            # zoneDriverCount[selectedOrder['startZoneID']-1] = zoneDriverCount[selectedOrder['startZoneID']-1] - 1
            sum = sum + 1

            for point in range(len(instance['drivers'][k]['temp_route']) - 1):
                if instance['drivers'][k]['temp_route'][point][5] < 3 or \
                        instance['drivers'][k]['temp_route'][point + 1][5] < 3:
                    l1 = instance['drivers'][k]['temp_route'][point][0]
                    l2 = instance['drivers'][k]['temp_route'][point + 1][0]
                    path1 = computePath.computePath(instance['graph'], instance['reverseGraph'], l1, l2)
                    # print(len(path1))
                    time_all = 0
                    for node in range(len(path1) - 1):
                        # print()
                        time_all = time_all + instance['edges'][str(path1[node])][str(path1[node + 1])][0]
                        TaxiDemandSupplyOracle.addTimeRecord(instance['taxiWatchdog'],
                                                             instance['node2region'][path1[node + 1]],
                                                             instance['drivers'][k]['temp_route'][point][
                                                                 2] + time_all, 1)




    # for k in range(len(instance['secondDrivers'])):
    #     zoneOrders = {}
    #     selectedOrder = {}
    #
    #
    #     # if instance['secondDrivers'][k]['servingOrder']['idletime'] < 50:
    #     #     continue
    #
    #
    #     for j in range(len(instance['orders'])):
    ...
    #                        instance['CHgraph'], instance['cacheSD'], instance['distanceFrequentNodes'],
    #                        instance['frequentPickup'], instance['frequentDrop'])
    #         if reachable(instance['orders'][j]["dist"], instance['orders'][j], instance['secondDrivers'][k], instance, detour_factor) == False:
    #             continue
    #         # print(str(instance['node2region'][instance['drivers'][k]['temp_route'][0][0]]), str(instance['orders'][j]["startZoneID"]))
    #         if instance['node2region'][instance['secondDrivers'][k]['temp_route'][0][0]] != instance['orders'][j]["startZoneID"]:
    #             if instance['time_list'][str(instance['node2region'][instance['secondDrivers'][k]['temp_route'][0][0]])][
    #                             str(instance['orders'][j]["startZoneID"])] <= instance['orders'][j]["startTime"] + instance['orders'][j]["maxWaitTime"] - instance['secondDrivers'][k]['temp_route'][0][2]:
    #                             continue
    #         zoneOrders[len(zoneOrders)] = instance['orders'][j]
    #     if zoneOrders == {}:
    #         continue
    #
    #     # print(k, len(zoneOrders))
    #     selectedOrder = BaseAlgorithm.ITG2findBestOrders2(instance['secondDrivers'][k], zoneOrders, zoneIdleTimeMap, oracle,
    #                                                 instance['taxiWatchdog'],
    #                                       currentTimeOffset, zoneDriverCount, 100000000000000000000000000000000, instance,0)
    #     # print(selectedOrder)
    #     if instance['secondDrivers'][k] != {} and selectedOrder != {}:
    #         selectedOrder['insert_idx'][0], selectedOrder['insert_idx'][1], cost, dist = Leinsertion(selectedOrder, instance['secondDrivers'][k], instance)
    #         if selectedOrder['insert_idx'][0] != -1:
    #             for point in range(len(instance['secondDrivers'][k]['temp_route'])-1):
    #                 if instance['secondDrivers'][k]['temp_route'][point][5] < 3 or instance['secondDrivers'][k]['temp_route'][point+1][5] < 3:
    #                     l1 = instance['secondDrivers'][k]['temp_route'][point][0]
    #                     l2 = instance['secondDrivers'][k]['temp_route'][point+1][0]
    #                     path1 = computePath.computePath(instance['graph'], instance['reverseGraph'], l1, l2)
    #                     # print(len(path1))
    #                     time_all = 0
    #                     for node in range(len(path1) - 1):
    #                         # print()
    #                         time_all = time_all + instance['edges'][str(path1[node])][str(path1[node + 1])][0]
    #                         TaxiDemandSupplyOracle.removeTimeRecord(instance['taxiWatchdog'], instance['node2region'][path1[node + 1]],
    #                                                                 instance['secondDrivers'][k]['temp_route'][point][2] + time_all, 1)
    #
    #             driver.serveOrder3(instance['secondDrivers'][k], selectedOrder, currentTimeOffset,
    #                             instance['velocity'], instance)
    #             # zoneOrders.pop(j)
    #             # assignedDrivers[len(assignedDrivers)] = instance['drivers'][k]
    #             for point in range(len(instance['secondDrivers'][k]['temp_route'])-1):
    ...
    #                         # print()
    #                         time_all = time_all + instance['edges'][str(path1[node])][str(path1[node + 1])][0]
    #                         TaxiDemandSupplyOracle.addTimeRecord(instance['taxiWatchdog'], instance['node2region'][path1[node + 1]],
    #                                                              instance['secondDrivers'][k]['temp_route'][point][2] + time_all, 1)
    #             # zoneIdleTimeMap.pop(selectedOrder['endZoneID'])
    #             zoneDriverCount[instance['node2region'][instance['secondDrivers'][k]['temp_route'][0][0]]] = zoneDriverCount[instance['node2region'][instance['secondDrivers'][k]['temp_route'][0][0]]] - 1
    #             sum = sum + 1
    #
    #
    #     if selectedOrder == {}:
    #         selectedOrder = BaseAlgorithm.ITGSortedfindBestOrders(instance['secondDrivers'][k], zoneOrders, zoneIdleTimeMap, oracle,
    #                                                         instance['taxiWatchdog'],
    #                                                         currentTimeOffset, zoneDriverCount,
    #                                                         100000000000000000000000000000000, instance, 0)
    #         # print(selectedOrder)
    #         if instance['secondDrivers'][k] != {} and selectedOrder != {}:
    #             selectedOrder['insert_idx'][0], selectedOrder['insert_idx'][1], cost, dist = Leinsertion2(
    #                 selectedOrder,
    #                 instance['secondDrivers'][k], instance)
    #             if selectedOrder['insert_idx'][0] != -1:
    #                 for point in range(len(instance['secondDrivers'][k]['temp_route']) - 1):
    #                     if instance['secondDrivers'][k]['temp_route'][point][5] < 3 or \
    #                             instance['secondDrivers'][k]['temp_route'][point + 1][5] < 3:
    #                         l1 = instance['secondDrivers'][k]['temp_route'][point][0]
    #                         l2 = instance['secondDrivers'][k]['temp_route'][point + 1][0]
    #                         path1 = computePath.computePath(instance['graph'], instance['reverseGraph'], l1, l2)
    #                         # print(len(path1))
    ...
    #                             # print()
    #                             time_all = time_all + instance['edges'][str(path1[node])][str(path1[node + 1])][0]
    #                             TaxiDemandSupplyOracle.removeTimeRecord(instance['taxiWatchdog'],
    #                                                                     instance['node2region'][path1[node + 1]],
    #                                                                     instance['secondDrivers'][k]['temp_route'][
    #                                                                         point][2] + time_all, 1)
    #
    #                 driver.serveOrder3(instance['secondDrivers'][k], selectedOrder, currentTimeOffset,
    #                                    instance['velocity'], instance)
    ...
    #                             TaxiDemandSupplyOracle.addTimeRecord(instance['taxiWatchdog'],
    #                                                                  instance['node2region'][path1[node + 1]],
    #                                                                  instance['secondDrivers'][k]['temp_route'][point][
    #                                                                      2] + time_all, 1)
    #                 # zoneIdleTimeMap.pop(selectedOrder['endZoneID'])
    #                 zoneDriverCount[instance['node2region'][instance['secondDrivers'][k]['temp_route'][0][0]]] = zoneDriverCount[instance['node2region'][instance['secondDrivers'][k]['temp_route'][0][0]]] - 1
    #                 sum = sum + 1

...
def varyingMaxWaitingTimes(seed):
    logger = MyLogger.MyLogger("varyingWaitingTime.txt", seed)
...
def varyFrameLength(seed):
    logger = MyLogger.MyLogger("varyingFrameLength.txt", seed)
...
def varyingLookupLengths(seed):
    logger = MyLogger.MyLogger("varyingLookupLength.txt", seed)
...
def loadProblem(info , day , diverCount, seed):
    data = ProblemInstanceLoader.loadProblemInstance(day, diverCount, seed)
    data["info"] = info
...
def runAlgorithms(data, frameLength):
    # round += 1
    tmpInstance = ProblemInstance.ProblemInstance(data)
    ...
    currentInstance = DataBatchProvider.fetchCurrentProblemInstance(tmpInstance, frameLength)
    while currentInstance != {}:
        ...
        if lastInstance != {}:
            ...
            print(len(tmpInstance['expiredOrders']))
            print(len(tmpInstance['completedOrders']))
            # print(j, len(currentInstance['drivers']),
            #       len(currentInstance['orders']),
            #       len(tmpInstance['expiredOrders']),
            #       len(tmpInstance['completedOrders']))
            j = j + 1
        BaseAlgorithm.run(currentInstance, j)
        print(driver.get_value('RealTotalDistance', 0))
        print(driver.get_value('OracleTotalDistance', 0))
        print(driver.get_value('TotalWaitTime', 0))
    ...
    tmpInstance["info"] = "IdleRatioGreedyAlgorithm" + ":" + tmpInstance["info"]
    tmpInstance["startRunningMillis"] = millisStart
    tmpInstance["endRunningMillis"] = millisEnd
    MyLogger.logResult(tmpInstance, 1)
